...
import pro.parseq.solvent.services.DispersedSequence;
...
@SpringBootTest
public class RemoteReferenceServiceTest {

	private static final Logger logger = LoggerFactory.getLogger(RemoteReferenceServiceTest.class);
	...
	private static final GenomicCoordinate testContigTwelfth = new GenomicCoordinate(testContig, TEST_CONTIG_LENGTH - 1);
	...
	private RemoteReferenceService refservice;

	@Before
	public void setUp() throws Exception {
		refservice = new RemoteReferenceService(config);
	}
	...
	public void testGapAwareCoordinateShifting() throws Exception {
		...
		assertThat(refservice.leftGapAwareShift(chr2StartNext, 0)).isEqualTo(chr2StartNext);
		assertThat(refservice.leftGapAwareShift(chr2StartNext, 1)).isEqualTo(chr2Start);
		assertThat(refservice.leftGapAwareShift(chr2StartNext, 2)).isEqualTo(chr1EndPrev);

		assertThat(refservice.rightGapAwareShift(chr1EndPrev, 0)).isEqualTo(chr1EndPrev);
		assertThat(refservice.rightGapAwareShift(chr1EndPrev, 1)).isEqualTo(chr1End);
		assertThat(refservice.rightGapAwareShift(chr1EndPrev, 2)).isEqualTo(chr2StartNext);
	}
}
